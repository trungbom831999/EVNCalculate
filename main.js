...
var loadImgButtonDevice = function () {
    ...
    for (var i = 0; i < listBtnDevice.length; i++) {
        ...
        listBtnDevice.eq(i).mouseover(function () {
            ...
            $(this).find(".iconBox").css('background-image', 'url(' + urlHover + ')');
            // console.log(urlHover);
        });
        listBtnDevice.eq(i).mouseout(function () {
            var urlBack = $(this).attr("backimg");
            $(this).find(".iconBox").css('background-image', 'url(' + urlBack + ')');
            // console.log(urlBack);
        });
    }
}

var btnDeviceClick = function () {
    var listBtnDevice = $('.btn-device');
    for (var i = 0; i < listBtnDevice.length; i++) {
        listBtnDevice.eq(i).click(function () {
            idBtnToChange = $(this).attr("data-id");
...
var addOrDeleteDevice = function (device, info) {
    var button = $("button[name='" + device + "']");
    var a = $("a[href='#" + device + "']");
    // var aInfo = $("a[href='#"+info+"']");
    var tab = $('#' + device);
    var listTabPane = $('#' + info).find('.tab-pane');


    button.click(function () {
        if (!button.hasClass('activeModal')) { //xoa
            a.css('display', 'none');
            if (a.hasClass('active')) {
                ...
                for (var i = 0; i < listTabPane.length; i++) {
                    ...
                    if (buttonChange.hasClass('activeModal')) {
                        listTabPane.eq(i).addClass('active show');
                        // console.log(navLink);
...
//check Active Modal
var checkActiveModal = function (device) {
...
var clickCheckActiveModal = function (device) {
    var listDevice = $('#list-' + device).find('.itemDevice');
    for (var i = 0; i < listDevice.length; i++) {
...
//check input is NaN
var checkInputValidate = function (device, info) { //check soGio, soLuong
    var button = $("button[name='" + device + "']");
    var a = $("a[href='#" + device + "']");
    var tab = $('#' + device);
    var listTabPane = $('#' + info).find('.tab-pane');
            ...
            if (!a.hasClass('active')) {
                for (var i = 0; i < listTabPane.length; i++) {
                    ...
                    if (listTabPane.eq(i).hasClass('active show')) {
                        listTabPane.eq(i).removeClass('active show');
                        // console.log(navLink);
...
var checkInputValidate2 = function (device, info) { 
    ...
    var tab = $('#' + device);
    var listTabPane = $('#' + info).find('.tab-pane');
    var soBong = $('#number-soBong-' + device);
    // var soLuong = $('#soLuong-' + device);
    if (button.hasClass('activeModal')) {
        if (soBong.val() == "") {
            if (!a.hasClass('active')) {
...
var checkInputValidate3 = function (device, info) { 
    ...
    var tab = $('#' + device);
    var listTabPane = $('#' + info).find('.tab-pane');
    var nhietDo = $('#temperature-' + device);
    // var soLuong = $('#soLuong-' + device);
    if (button.hasClass('activeModal')) {
...
var validateInputModal = function () {
...
$(document).ready(function () {
    ...
    $('.temperature-widget-wrapper').each(function () {
        ...
        btnUp.click(function () {
            ...
            spinner.find("input").val(newVal.toString());
            // $('#temperature-dieuHoa').val(newVal);
            // spinner.find("input").trigger("change");
        });
    ...
    addOrDeleteDevice('dieuHoaCay', info);
    // addOrDeleteDevice('quatCay', info);

    checkActiveModal('dieuHoa');
    clickCheckActiveModal('dieuHoa');

    // $('.rs-tooltip').attr('id', 'soGioDieuHoa');
